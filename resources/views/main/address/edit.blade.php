...
@section("content")
    <div class="container">
        <h2 class="text-center font-weight-bold" style="margin-top:80px;">Edit Address Anda</h2>
        <form action="{{route('address.update', $address->id)}}" method="POST">
            @csrf
            <div class="form-group col-6" style="margin-top:40px;">
                <label for="address">Address</label>
                <textarea name="address" id="address" cols="30" rows="8" placeholder="Address" class="form-control">{{$address->address}}</textarea>
            </div>
